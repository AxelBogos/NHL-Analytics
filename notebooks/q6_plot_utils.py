...
def compute_league_avg(df: pd.DataFrame, year: int) -> np.array:
    ...
    data_league = data_league / (season_hours * 2)
...
def compute_team_avg(df, year: int, team: str) -> np.array:
    ...
    team_matchs = df_copy.loc[
        (df_copy["home_team"] == team) | (df_copy["away_team"] == team)
        ]
    team_matchs_drop = team_matchs.drop_duplicates(
...
def all_season_all_team_avg(df: pd.DataFrame, start_year: int = 2016, end_year: int = 2020, sigma: int = 4,
    ...
    for year in range(start_year, end_year + 1):
        ...
        league_avg = compute_league_avg(df, year)
        for team in teams_per_season[str(year)]:
            team_avg = compute_team_avg(df, year, team)
            ...
            # Store result
            shot_freq_per_team_per_season[str(year)][team] = test_total_fine
...
def get_half_rink_projection_df() -> pd.DataFrame:
    ...
    """
    half_rink_df = pd.read_csv(os.path.join(DATA_DIR, "tidy_data.csv"))
